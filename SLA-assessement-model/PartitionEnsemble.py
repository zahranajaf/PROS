...
from plot_domain import plot_2d_domains
from plot_domain import plot_multiplex_community as plot_multilayer_pd
...
sub
    ...
    def _add_partitions_fast(self, listofparts, maxpt=None):
        ...
        t = time()
        if self.ismultilayer:
            partitions, int_edges, exp_edges, int_inter_edges, resolutions, couplings, orig_mods = zip(*[(part[
                                                                                                              'partition'],
                                                                                                          part[
                                                                                                              'int_edges'],
                                                                                                          part[
            ...
            print(self.int_edges)
            self.exp_edges = np.array(exp_edges)
            print(self.exp_edges)
            self.resolutions = np.array(resolutions)
            print(self.resolutions)
            self.int_inter_edges = np.array(int_inter_edges)
            print(self.int_inter_edges)
            self.couplings = np.array(couplings)
            print(self.couplings)
        else:
            partitions, int_edges, exp_edges, resolutions, orig_mods = zip(
                *[(part['partition'], part['int_edges'], part['exp_edges'], part['resolution'], part['orig_mod']) for
                  part in listofparts])
            self._partitions = np.array(partitions)
            self.int_edges = np.array(int_edges)
            self.exp_edges = np.array(exp_edges)
        ...
        self.numcoms = np.apply_along_axis(lambda x: get_number_of_communities(x, min_com_size=self._min_com_size),
                                           arr=self._partitions, axis=1)
        ...
        if self._hdf5_file is not None:
            ...
            for part in partitions:
                ...
                if "orig_mod" in part:
                    ...
                    # calculated original modularity from orig resolution
                    self.orig_mods = np.append(self.orig_mods,
                                               self.int_edges[-1] - self.resolutions[-1] * self.exp_edges)
                else:
                    self.orig_mods = np.append(self.orig_mods, None)
    ...
    def merge_ensemble(self, otherEnsemble, new=True):
        ...
        if new:
            ...
            if self.ismultilayer:
                attributes += ['couplings', 'int_inter_edges']
            for attribute in attributes:
                newEnsemble.__dict__[attribute] = np.concatenate(
                    [newEnsemble.__dict__[attribute], otherEnsemble.__dict__[attribute]])
    ...
    def get_coefficient_array(self, subset=None):
        ...
        if len(subset) == 0:
            warnings.warn(
                "Zero sized subset requested.  Returning empty array.  self.num_parts={:}".format(self.num_parts))
    ...
    def _write_graph_to_hd5f_file(self, file, compress=4, intra=True):
        ...
        grph.create_dataset("edge_list",
                            data=np.array([e.tuple for e in graph2save.es]), compression="gzip",
        ...
        for attrib in graph2save.edge_attributes():
            if is_py3 and type(graph2save.es[attrib][0]) is str:
                ...
                edge_atts.create_dataset(attrib,
                                         data=np.array(graph2save.es[attrib]), compression="gzip",
                                         compression_opts=compress)
        ...
        if hdf5:
            ...
            if self._isonfile:
                ...
                if self._hdf5_file == filename:
                    ...
                    warnings.warn(
                        "PartitionEnsemble object is already stored in file: {:}. Moving to new file: {:}".format(
    ...
    def plot_modularity_mapping(self, ax=None, downsample=2000, champ_only=False, legend=True,
        ...
        if champ_only:
            ...
            leg_text = ['modularity', r'\# communities ($\ge %d $ nodes)' % (self._min_com_size),
                        "transitions,$\gamma$",
                        r"\# communities ($\ge %d$ nodes) optimal" % (self._min_com_size), "convex hull of $Q(\gamma)$"]
        if legend:
            l = ax.legend(leg_set,
    ...
    def _split_doms_df(self, doms_df, allgammas):
        ...
        def add_new_value(df, value):
            ...
            if len(inds) == 0:  # nothing is less than, add to beginning of df
                ...
                curind, curstart, curend = df.iloc[inds[-1], :][
                    ['ind', 'start_gamma', 'end_gamma']]  # last value that is smaller
    ...
    def get_ami_over_gamma_curve(self, otherEnsemble):
        ...
        dom_df.loc[:, 'self'] = 1
        dom_df2.loc[:, 'self'] = 0
        assert dom_df.shape[0] == dom_df2.shape[0], "Uneqaul combined domains lengths : {:d}!={:d}".format(
            dom_df.shape[0], dom_df2.shape[0])
